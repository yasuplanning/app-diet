...
export function buildExport(start, end, scope = 'all') {
  ...
  if (wantMasters) {
    const foods = db.prepare('SELECT * FROM foods ORDER BY id').all();
    const supplements = db.prepare('SELECT * FROM supplements ORDER BY id').all();
  ...
  if (wantRecords) {
    const meals = db.prepare('SELECT * FROM meals WHERE date >= ? AND date <= ? ORDER BY date, time, id').all(s, e);
    const energy = db.prepare('SELECT * FROM daily_energy WHERE date >= ? AND date <= ? ORDER BY date').all(s, e);
    const body = db.prepare('SELECT * FROM body_records WHERE date >= ? AND date <= ? ORDER BY date').all(s, e);
    const supplementLogs = db.prepare('SELECT * FROM supplement_logs WHERE date >= ? AND date <= ? ORDER BY date, time, id').all(s, e);
...
export function applyImport(zipBuffer, opts = {}) {
  ...
  try {
    ...
    const insertFood = db.prepare(`INSERT INTO foods (${foodCols.join(',')}) VALUES (${foodCols.map(() => '?').join(',')})`);
    const nameToId = {};
    for (const f of impFoods) {
      const existing = db.prepare('SELECT * FROM foods WHERE name = ?').get(f.name);
      if (existing) {
        ...
        const info = insertFood.run(...params);
    ...
    for (const m of impMeals) {
      const dup = db.prepare('SELECT id FROM meals WHERE date=? AND time=? AND mealType=? AND foodName=? AND grams=?')
      ...
      if (!foodId) {
        const f = db.prepare('SELECT id FROM foods WHERE name = ?').get(m.foodName);
        foodId = f ? f.id : null;
      }
      insertMeal.run(m.date, m.time ?? '', m.mealType ?? 'その他', foodId, m.foodName, m.grams, m.memo ?? '', foodId ? 0 : 1, now(), now());
      summary.meals.added++;
    }

    // --- daily_energy（日付キー）---
    for (const en of impEnergy) {
      const existing = db.prepare('SELECT date FROM daily_energy WHERE date = ?').get(en.date);
      if (existing) {
        if (mode === 'overwrite') {
          db.prepare('UPDATE daily_energy SET burnedCalories=?, updatedAt=? WHERE date=?').run(en.burnedCalories ?? null, now(), en.date);
          summary.daily_energy.updated++;
        } else summary.daily_energy.skipped++;
      } else {
        db.prepare('INSERT INTO daily_energy (date, burnedCalories, updatedAt) VALUES (?, ?, ?)').run(en.date, en.burnedCalories ?? null, now());
    ...
    for (const b of impBody) {
      const existing = db.prepare('SELECT id FROM body_records WHERE date = ?').get(b.date);
      if (existing) {
        if (mode === 'overwrite') {
    ...
    const insertSupp = db.prepare(`INSERT INTO supplements (${suppCols.join(',')}) VALUES (${suppCols.map(() => '?').join(',')})`);
    const suppNameToId = {};
    for (const s of impSupplements) {
      const existing = db.prepare('SELECT * FROM supplements WHERE name = ?').get(s.name);
      if (existing) {
        ...
        const info = insertSupp.run(...params);
    ...
    for (const l of impSupplementLogs) {
      const dup = db.prepare('SELECT id FROM supplement_logs WHERE date=? AND time=? AND supplementName=? AND units=?')
      ...
      if (!suppId) {
        const s = db.prepare('SELECT id FROM supplements WHERE name = ?').get(l.supplementName);
        suppId = s ? s.id : null;
      }
      insertSuppLog.run(l.date, l.time ?? '', suppId, l.supplementName, l.units, l.memo ?? '', now(), now());
      summary.supplement_logs.added++;
    }

    // --- goals（任意）---
    if (importGoals && impGoals) {
      db.prepare('UPDATE goals SET targetCalories=?, targetProtein=?, targetFiber=?, saltLimit=?, weightGoal=?, bodyFatGoal=?, updatedAt=? WHERE id=1')
